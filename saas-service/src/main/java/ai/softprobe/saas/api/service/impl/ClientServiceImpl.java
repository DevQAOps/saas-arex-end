...
import ai.softprobe.saas.common.configuration.AdminConfig;
import ai.softprobe.saas.api.common.enums.ErrorCode;
import ai.softprobe.saas.api.common.exceptions.SpSaasException;
import ai.softprobe.saas.api.service.ClientOauthService;
import ai.softprobe.saas.api.service.ClientService;
import ai.softprobe.saas.api.service.jwt.ApiSystemAuthJwtService;
import ai.softprobe.saas.api.model.contract.ClientDownloadResponse;
import ai.softprobe.saas.api.model.contract.ClientDownloadResponse.InnerClass;
...
import ai.softprobe.saas.api.model.mapper.ClientOauthInfoMapper;
...
public class ClientServiceImpl implements ClientService {
  ...
  private final ApiSystemAuthJwtService apiSystemAuthJwtService;
  ...
  public ClientOauthResponse clientLogin(ClientOauthRequest clientOauthRequest) {
    ...
    if (StringUtils.isEmpty(email)) {
      throw new SpSaasException(ErrorCode.CLIENT_LOGIN_PROVIDER_NOT_SUPPORTED.getCodeValue(),
          "Failed to get email from oauth provider");
    }

    // verify whether the email has been verified
    if (!clientOauthResultDto.isEmailVerified()) {
      throw new SpSaasException(ErrorCode.CLIENT_LOGIN_EMAIL_NOT_VERIFIED.getCodeValue(),
    ...
    if (CollectionUtils.isEmpty(tenantInfos)) {
      throw new SpSaasException(ErrorCode.CLIENT_LOGIN_USER_NOT_FOUND.getCodeValue(),
          "No tenant found for email");
    }

    String authToken = apiSystemAuthJwtService.makeApiAuthToken(email);
  ...
  private ClientOauthService getClientOauthService(Integer code) {
    return clientOauthServices.stream().filter(
        ...
        .findFirst()
        .orElseThrow(() -> new SpSaasException(
