package ai.softprobe.saas.storage.api.interceptor;
...
import ai.softprobe.saas.common.interceptor.TenantRateLimitPerClientInterceptor;
import ai.softprobe.saas.common.interceptor.TenantTrafficLimitInterceptor;
import ai.softprobe.saas.common.repository.SaasSystemConfigurationRepository;
import ai.softprobe.saas.common.repository.impl.UsageStatDao;
...
public class InterceptorHandlerAutoConfiguration {
  ...
  private List<String> getAuthorizationExcludePathPatterns() {
    return Lists.newArrayList("/error", "/favicon.ico", "/vi/health", "/api/config/agent/**",
        "/api/storage/record/**", "/api/rr/record");
  }
